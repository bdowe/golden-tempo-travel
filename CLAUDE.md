...
- `plan_handler.go` — Streaming SSE endpoint (`POST /api/v1/plan`) backed by `anthropic-sdk-go`; agent loop (claude-sonnet-4-6) dispatches through the ordered tool registry in `plan_tool_registry.go`; `plan_compactor.go` summarizes long histories with Haiku 4.5; reads `ANTHROPIC_API_KEY` env var
